...
_RECOVERY_FILE_NOT_BASE64_WARNING = (
    'Warning: server returned a recoveryFile that is not valid base64; '
    'it was NOT saved to disk. Please contact Circle support to obtain a '
    'valid recovery file for the registered entity secret.'
)
...
def register_entity_secret_ciphertext(api_key: str, entity_secret: str, recoveryFileDownloadPath: str = '', base_url: str = "https://api.circle.com") -> Dict[str, Any]:
    ...
    try:
        ...
        if 'recoveryFile' in response_data['data']:
            recovery_file = response_data['data']['recoveryFile']
            if isinstance(recovery_file, str) and re.fullmatch(base64_regex, recovery_file):
                ...
                print(entity_secret_message)
                print(f'{top_bottom_border}\n')
                _save_recovery_file(recovery_file, recoveryFileDownloadPath)
            else:
                print(_RECOVERY_FILE_NOT_BASE64_WARNING)
                # Remove the unsaved value so callers can detect the skip via
                # `'recoveryFile' not in result['data']`.
                response_data['data'].pop('recoveryFile')
